    ...
    const BlogCardItem = ({ post }) => {
        ...
        return (
            <article className="blog-card inline-flex flex-col items-center gap-4  relative flex[0_0_auto] bg-white rounded-[5px] overflow-hidden shadow-[0px_10px_50px_#0000001a">
                ...
                <img src={post.image} alt={post.title} className="blog-image object-cover rounded-[5px]" />
                <div className="blog-content flex items-center gap-2.5 relative self-stretch w-full flex-[0_0_auto]">
                    <time className="relative w-fit font-normal text-[#0d0c12] text-base text-center tracking-[0] leading-[normal]">{post.date}</time>
                    <img className="relative w-px h-[25px] object-cover" alt="Divider" src={post.divider}/>
                    <span className="relative w-fit font-normal text-[#0d0c12] text-base text-center tracking-[0] leading-[normal]">{post.readTime}</span>
                </div>
                <div>
                    <h3>{post.title}</h3>
                ...
                <div className="flex items-center gap[13px] relative self-stretch w-full flex-[0_0_auto]">
                    <a href="#" className="inline-flex items-center justify-center gap-2.5 relative flex-[0_0_auto] rounded-[5px] overflow-hidden hover:opacity-80 transition-opacity" aria-label={`Read blog post: ${post.title}`}>
                        <span className="relative w-fit mt-[-1.00px] font-semibold text-[#801323] text-base tracking-[0] leading-[normal]">
                            Read Blog
                        </span>
                        <div className="relative w-5 h-5">
                            <img className="absolute w-[20px] h-[20px] left-0.5" alt="open link" src={post.icon}/>
                        </div>
                    </a>
                </div>
            </article>
